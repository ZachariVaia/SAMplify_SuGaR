        ...
        if not use_centers_to_extract_mesh:
            ...
            for surface_level in surface_levels:
                ...
                for decimation_target in decimation_targets:
                    ...
                    # Example debug log
                    CONSOLE.print(f"Foreground mesh exists: {decimated_o3d_fg_mesh is not None}")
                    CONSOLE.print(f"Background mesh exists: {decimated_o3d_bg_mesh is not None}")

                    if decimated_o3d_fg_mesh is not None:
                        CONSOLE.print("Using foreground mesh only, since background mesh is empty.")
                        decimated_o3d_mesh = decimated_o3d_fg_mesh
                    else:
                        raise ValueError("Foreground mesh is empty. Please provide a valid bounding box for the scene.")

                    # Add more detailed logs if necessary
